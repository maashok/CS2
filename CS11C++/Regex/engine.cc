...
// Returns range object indicating where match occurred
Range find(vector<RegexOperator *> regex, const string &s) {
	// Try starting to find the regex at every index of string
	for (unsigned int i = 0; i < s.size(); i++) {
		// Try finding at a specific index
		Range match = findAtIndex(regex, s, i);
		// If it matches for some range, return the match
...
bool match(vector<RegexOperator *> regex, const string &s) {
	// Try to find the regex in the string
	Range matchFind = find(regex, s);
	// If the regex starts at the first index and ends at the last
	// then it matches
	if (matchFind.start == 0 && unsigned(matchFind.end) == s.size())
		return true;
	else
		return false;
}
